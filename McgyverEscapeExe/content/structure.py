...
""" Class that permits to generate and load the labyrinth, the adjacent frame and the inventory via methods"""
class Structure():
		...
		for element in json.load(open(self.file)):
			for i in element:
				if i == "w":
					...
					self.cropped_wall=pygame.transform.scale(self.cropped_wall,(sprite_size,sprite_size))
	...
	def load_field(self,sprite_number_s,line_number_s):
		...
		self.cropped_field=background.subsurface(background_field)
		self.cropped_field=pygame.transform.scale(self.cropped_field,(sprite_size,sprite_size))
		self.main_window.blit(self.cropped_field,(sprite_number_s * sprite_size,line_number_s * sprite_size))		
	...
	def load_start(self,sprite_number_s,line_number_s):
		...
		self.cropped_start=background.subsurface(background_start)
		self.cropped_start=pygame.transform.scale(self.cropped_start,(sprite_size,sprite_size))
		self.main_window.blit(self.cropped_start,(sprite_number_s * sprite_size,line_number_s * sprite_size))
	...
	def load_frame(self):
		...
		self.frame_background=pygame.transform.scale(self.frame_background,
			(window_side_x - frame_side,window_side_y))
	...
	def load_victory(self,victory_picture):
		...
		self.victory_picture=pygame.transform.scale(self.victory_picture,(frame_side,window_side_y))
	...
	def load_defeat(self,defeat_picture,artwork):
		...
		self.defeat_picture=pygame.transform.scale(self.defeat_picture,(frame_side,window_side_y))
